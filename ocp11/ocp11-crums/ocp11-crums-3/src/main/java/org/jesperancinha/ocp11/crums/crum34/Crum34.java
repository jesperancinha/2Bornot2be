...
public class Crum34 {
    ...
    public static void main(String[] args) {
        ...
        Arrays.stream(armonaBoat.getClass()
            .getAnnotations()).forEach(Consolerizer::printBrightCyanGenericLn);
        printMagentaGenericLn("For the %s boat we have:", tourBoat.getClass());
        Arrays.stream(tourBoat.getClass()
            .getAnnotations()).forEach(Consolerizer::printBrightCyanGenericLn);

        printGreenGenericLn("We need to be aware that although we made our annotation Boat repeatable, it is not dependent on Boats");
        printGreenGenericLn("The code declaration of Boats allow us to use the Boat annotation independently and repeatedly");
